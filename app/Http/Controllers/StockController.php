...
class StockController extends Controller
{
    ...
    public function stockstore(Request $request)
    {
        ...
        $stock->pack= $request->pack;
        $stock->openningstock=$request->openningstock;
        $stock->pricelp= $request->pricelp;
        $stock->price= $request->price;
        $stock->stock= $request->stock;
        $stock->remainingstock= $stock->stock + $request->remainingstock;
        
                /*$torders = DailySales::selectRaw('sum(orders)')
                ->where('pname','=', $request->title)
                ->get();
                
                 $treturnpcs = DailySales::selectRaw('sum(returnc)')
                ->where('pname','=', $request->title)
                ->get();
                
                $stock->remainingstock = ($stock->stock +  $treturnpcs->sum(returnc)) - $torders->sum(orders);*/
    ...
    public function stockeditprocess(Request $request, $id)
    {
        ...
        //$data->remainingstock= $request->remainingstock;
